...
$query = "
    SELECT pr.nama_menu, SUM(DISTINCT ol.total_promo) AS total_promo, COUNT(DISTINCT ol.OrderID) AS total_qty_used
    FROM promo pr
    JOIN order_list ol ON pr.PromoID = ol.PromoID
...
switch ($sortOption) {
    case 'idr_highest':
        $orderBy = "ORDER BY total_promo DESC";
        break;
    case 'idr_lowest':
        $orderBy = "ORDER BY total_promo ASC";
        break;
    case 'qty_highest':
    ...
    case 'qty_lowest':
        ...
        break;
    default:
        $orderBy = "ORDER BY total_promo DESC"; // Default sorting
}

// Apply sorting
$query .= " GROUP BY pr.nama_menu $orderBy";
...
<head>
    ...
    <title>Detail Penggunaan Promo</title>
...
<body>
    ...
    <div class="header">
            <h2><i class='bx bx-left-arrow-alt' onclick="goBack()"></i>Detail Penggunaan Promo</h2>
    </div>
    
        <h4>Lihat promo apa saja yang terpakai</h4>
        <div class="filter-bar">
        <form action="detailp.php" method="GET" id="filter-form">
            <div>
                <h3>Sort By</h3>
        ...
        <tbody>
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td>" . number_format($row['total_promo'], 0, ',', '.') . "</td>";
