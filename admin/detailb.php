...
$query = "
    SELECT b.nama_menu, SUM(DISTINCT ol.total_promo) AS total_bundling, COUNT(DISTINCT ol.OrderID) AS total_qty_used
    FROM bundling b
    JOIN order_list ol ON b.BundlingID = ol.BundlingID
...
switch ($sortOption) {
    case 'idr_highest':
        $orderBy = "ORDER BY total_bundling DESC";
        break;
    case 'idr_lowest':
        $orderBy = "ORDER BY total_bundling ASC";
        break;
    case 'qty_highest':
    ...
    case 'qty_lowest':
        ...
        break;
    default:
        $orderBy = "ORDER BY total_bundling DESC"; // Default sorting
}

// Apply sorting
$query .= " GROUP BY b.nama_menu $orderBy";
...
<head>
    ...
    <title>Detail Penggunaan Bundling</title>
...
<body>
    ...
    <div class="header">
            <h2><i class='bx bx-left-arrow-alt' onclick="goBack()"></i>Detail Penggunaan Bundling</h2>
    </div>
        <h4>Lihat performa dari setiap bundling</h4>
        <div class="filter-bar">
        <form action="detailb.php" method="GET" id="filter-form">
            <div>
                <h3>Sort By</h3>
                <select id="sort_by" name="sort_by" onchange="this.form.submit()">
                    <option value="idr_highest">Sort by IDR: Highest - Lowest</option>
                    <option value="idr_lowest">Sort by IDR: Lowest - Highest</option>
                    <option value="qty_highest">Sort by Quantity: Highest - Lowest</option>
                    <option value="qty_lowest">Sort by Quantity: Lowest - Highest</option>
                </select>
            </div>
    ...
    <table class="reports-table">
        <thead>
            <tr>
                ...
                <th>Total Qty Bundling Terpakai</th>
            </tr>
        </thead>
        <tbody>
            <?php
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td>" . number_format($row['total_bundling'], 0, ',', '.') . "</td>";
