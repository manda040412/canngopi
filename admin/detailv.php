...
$query = "
    SELECT 
        v.Voucher,
        SUM(DISTINCT ol.total_promo) AS total_voucher,
        COUNT(DISTINCT CONCAT(v.VoucherID, '_', DATE(p.date))) AS total_qty_used
    FROM 
        voucher v
    JOIN order_list ol ON v.VoucherID = ol.VoucherID
...
switch ($sortOption) {
    case 'idr_highest':
        $orderBy = "ORDER BY total_voucher DESC";
        break;
    case 'idr_lowest':
        $orderBy = "ORDER BY total_voucher ASC";
        break;
    case 'qty_highest':
    ...
    case 'qty_lowest':
        ...
        break;
    default:
        $orderBy = "ORDER BY total_voucher DESC"; // Default sorting
}
...
// Store debugging info for console
$debug_info = [
    'query' => $debug_query,
    'date_filter' => [
...
<head>
    ...
    <title>Detail Penggunaan Voucher</title>
...
<body>
    ...
    <div class="header">
            <h2><i class='bx bx-left-arrow-alt' onclick="goBack()"></i>Detail Penggunaan Voucher</h2>
    </div>
    
        <h4>Lihat berapa banyak voucher yang terpakai</h4>
        <div class="filter-bar">
        <form action="detailv.php" method="GET" id="filter-form">
            <div>
                <h3>Sort By</h3>
    ...
    <table class="reports-table">
        <thead>
            <tr>
                ...
                <th>Total Qty Voucher Terpakai</th>
            </tr>
        </thead>
        <tbody>
            <?php
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td>" . number_format($row['total_voucher'], 0, ',', '.') . "</td>";
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        console.log(<?php echo $debug_json; ?>);
    });
    </script>
    ...
    <script>
        ...
        // Additional debug info about query results
        console.log('Query Results:');
        console.log('Number of rows returned: <?php echo mysqli_num_rows($result); ?>');
        ...
        // Reset result pointer again for the actual display
        mysqli_data_seek($result, 0);
        ?>
        
        console.log('Query Results Data:');
        console.log(<?php echo json_encode($debug_rows); ?>);
    </script>
</body>
